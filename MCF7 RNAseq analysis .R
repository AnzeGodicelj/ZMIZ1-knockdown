...
library("gplots")
library(topGO)
setwd("/Users/AnzeGodicelj/Library/Mobile Documents/com~apple~CloudDocs/Documents/rnaseq")
#List BAM files
FilesToCount <- dir("/Users/AnzeGodicelj/Library/Mobile Documents/com~apple~CloudDocs/Documents/rnaseq/SLX-13365", pattern = '.bam$', full.names = TRUE)
FilesToCount 
...
# Great! Now we can do cthe cool stuff - like converting entrez ids to gene symbols!!! Amazing, right?!
...
SLX_ID <- read.csv("/Users/AnzeGodicelj/Library/Mobile Documents/com~apple~CloudDocs/Documents/rnaseq/SLX_ID.csv", as.is = TRUE)
SLX_ID
...
write.csv(named_newcounts, file = "named newcounts.csv")

#Creating DESeqDataSet
load('named_newcounts.rda')

#Delete names of rownames of table replicate_newcounts

names(rownames(named_newcounts)) <- NULL

# Creating experimental data matrix  with cell line names, passages and treatments (row names of experimantal data
# are in the same order as names of the columns in matrix named_newcounts).
...
par(las=1)
plotMA(MCF7_MA_data, main = 'DESeq2 MCF7 MA Plot', alpha=0.01)
...
MCF7_significant_genes <- rownames(MCF7_MA_results_table)[which(MCF7_MA_results_table$padj<0.01)] # ajdust the p value for less recults
...
png("PCA on MCF7 cell line (non-scaled MCF7 data only).png",width=10000,height=8000,pointsize=60, res = 300)
plot(MCF7_pca$rotation[,1],MCF7_pca$rotation[,2],
     ...
     xlab = paste0("PC1 (",signif(pcavar[1],3),"%)"), 
     ylab = paste0("PC2 (",signif(pcavar[2],3),"%)"))
...
png("PC2 vs PC3 on MCF7 cell line (non-scaled MCF7 data only).png",width=10000,height=8000,pointsize=60, res = 300)
plot(MCF7_pca$rotation[,2],MCF7_pca$rotation[,3],
     ...
     lwd = 2,
     main = "Principal component analysis on MCF7 cell line", 
     xlab = paste0("PC2 (",signif(pcavar[2],3),"%)"), 
     ylab = paste0("PC3 (",signif(pcavar[3],3),"%)"))
...
# Obtain our MCF7_signature
View(MCF7_MA_results_table)
MCF7_signature <- MCF7_MA_results_table$stat
names(MCF7_signature) <- rownames(MCF7_MA_results_table)
MCF7_signature
...
reactomepathways <- msigDBsymbol$c2.cp.reactome.v5.0.symbols.gmt

# Repeat this block of code for every pathway in the allpathways list
if(!file.exists("MCF7_GSEA_allpahways.rda"))
{
  MCF7_GSEAresults<-matrix(0,nrow=0,ncol=3)
  colnames(MCF7_GSEAresults)<-c("Pathway Name","NES","pvalue")
  ...
  for(pathwayname in names(collection)){
    ...
    # Calculate GSEA
    obj<-gsea(reflist=MCF7_signature,set=pathway,method="pareto")
    nes<-obj$nes
    pvalue<-obj$p.value
    
    entry<-c(pathwayname,nes,pvalue)
    MCF7_GSEAresults<-rbind(MCF7_GSEAresults,entry)
    
    setTxtProgressBar(pb,nrow(MCF7_GSEAresults))
    save(MCF7_GSEAresults,file="MCF7_GSEA_allpahways.rda")
  }}else{
    load ("MCF7_GSEA_allpahways.rda")
  }


write.csv(MCF7_GSEAresults, file = "MCF7_GSEA_allpahways.csv")


# Adjust your pvalues
MCF7_adjusted<-p.adjust(MCF7_GSEAresults[,"pvalue"],method="bonferroni")
adjusted_MCF7_GSEAresults<-cbind(MCF7_GSEAresults,MCF7_adjusted)
write.table(adjusted_MCF7_GSEAresults,file="adjusted_MCF7_GSEA_allpahways")
save(adjusted_MCF7_GSEAresults, file = "adjusted_MCF7_GSEA_allpahways.rda")
write.csv(adjusted_MCF7_GSEAresults, file = "adjusted_MCF7_GSEA_allpahways.csv")
...
# Repeat this block of code for every pathway in the REACTOMEpathways list
if(!file.exists("MCF7 GSEA reactome pahways.rda"))
{
  MCF7_reactome_GSEAresults<-matrix(0,nrow=0,ncol=3)
  colnames(MCF7_reactome_GSEAresults)<-c("Pathway Name","NES","pvalue")
  ...
  for(pathwayname in names(collection)){
    ...
    entry<-c(pathwayname,nes,pvalue)
    MCF7_reactome_GSEAresults<-rbind(MCF7_reactome_GSEAresults,entry)
    
    setTxtProgressBar(pb,nrow(MCF7_reactome_GSEAresults))
    save(MCF7_reactome_GSEAresults,file="MCF7 GSEA reactome pahways.rda")
  }}else{
    load ("MCF7 GSEA reactome pahways.rda")
  }

write.csv(MCF7_reactome_GSEAresults, file = "MCF7 GSEA reactome pathways.csv")



# Adjust your pvalues
MCF7_reactome_adjusted<-p.adjust(MCF7_reactome_GSEAresults[,"pvalue"],method="bonferroni")
adjusted_MCF7_reactome_GSEAresults<-cbind(MCF7_reactome_GSEAresults,MCF7_reactome_adjusted)
write.table(adjusted_MCF7_reactome_GSEAresults,file="adjusted_MCF7_reactome_GSEAresults")
save(adjusted_MCF7_reactome_GSEAresults, file = "adjusted_MCF7_reactome_GSEAresults.rda")
write.csv(adjusted_MCF7_reactome_GSEAresults, file = "adjusted_MCF7_reactome_GSEAresults.csv")
